...
def update_readings():
    if pinger is not None and ping_timer.hasPeriodPassed(0.05):  # radars can update up to every 50 ms
        ...
        global prev_time
        time_elapsed = speed_timer.get() - prev_time
        if time_elapsed > 0.1:  # update speeds every 0.1 seconds
            global speeds, distances
            speeds = [(distances[i] - new_dists[i]) / time_elapsed for i in range(len(speeds))]
            prev_time = speed_timer.get()
        distances = new_dists
